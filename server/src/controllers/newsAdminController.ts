...
type ManagedNewsStatus = 'pending_review' | 'duplicate_review' | 'draft' | 'published' | 'scheduled' | 'rejected';

const NEWS_STATUSES = new Set<ManagedNewsStatus>(['pending_review', 'duplicate_review', 'draft', 'published', 'scheduled', 'rejected']);
const ALLOWED_TRANSITIONS: Record<ManagedNewsStatus, ManagedNewsStatus[]> = {
...
const ensureStatus = (
  status: unknown,
  fallback: ManagedNewsStatus = 'draft'
) => {
  const value = String(status || '').trim();
  return NEWS_STATUSES.has(value as ManagedNewsStatus) ? (value as ManagedNewsStatus) : fallback;
};

const canTransition = (from: ManagedNewsStatus, to: ManagedNewsStatus) =>
  (ALLOWED_TRANSITIONS[from] || []).includes(to);

const buildUniqueSlug = async (title: string, currentId?: string) => {
  ...
  while (true) {
    ...
    if (!found || String(found._id) === String(currentId || '')) break;
    slug = `${base}-${count}`;
    count += 1;
  }
  return slug;
...
const getActorId = (req: Request) => (req as Request & { adminActorId?: string }).adminActorId || '';

const resolveEditPayload = async (req: Request, itemId?: string) => {
  const body = req.body || {};
...
const pushCsvOrXlsx = (res: Response, filename: string, rows: Array<Record<string, unknown>>, type: 'csv' | 'xlsx') => {
...
export const getAdminNewsList = async (req: Request, res: Response) => {
  ...
  if (String(req.query.isAiSelected || '') === 'true') filter.isAiSelected = true;
  if (q) {
    filter.$or = [
...
export const getAdminNewsById = async (req: Request, res: Response) => {
  ...
  if (!item) return sendError(res, 'NOT_FOUND', 'News item not found', 404);
  return sendSuccess(res, { item });
};

export const createAdminNews = async (req: Request, res: Response) => {
  const payload = await resolveEditPayload(req);
  if (!payload.title) return sendError(res, 'VALIDATION_ERROR', 'title is required');
...
export const updateAdminNews = async (req: Request, res: Response) => {
  ...
  if (!before) return sendError(res, 'NOT_FOUND', 'News item not found', 404);
...
export const deleteAdminNews = async (req: Request, res: Response) => {
  ...
  if (!deleted) return sendError(res, 'NOT_FOUND', 'News item not found', 404);
  await writeNewsAuditLog(req, 'delete', 'news', String(req.params.id), { before: { title: deleted.title, status: deleted.status } });
  return sendSuccess(res, { deleted: true });
};

const publishItem = async (req: Request, res: Response, itemId: string) => {
  ...
  const now = new Date();
  const updated = await NewsItemModel.findByIdAndUpdate(
    itemId,
    {
      $set: {
        status: 'published',
...
export const approvePublishAdminNews = async (req: Request, res: Response) => publishItem(req, res, req.params.id);

export const rejectAdminNews = async (req: Request, res: Response) => {
  const item = await NewsItemModel.findById(req.params.id).lean();
  if (!item) return sendError(res, 'NOT_FOUND', 'News item not found', 404);
  ...
  const updated = await NewsItemModel.findByIdAndUpdate(
    req.params.id,
    { $set: { status: 'rejected', approvedByAdminId: getActorId(req) || null } },
    { new: true }
  ).lean();
...
export const scheduleAdminNews = async (req: Request, res: Response) => {
  const settings = await getNewsSettings();
  ...
  if (!scheduleAtRaw) return sendError(res, 'VALIDATION_ERROR', 'scheduledAt is required');
  const scheduledAt = new Date(scheduleAtRaw);
  if (Number.isNaN(scheduledAt.getTime())) return sendError(res, 'VALIDATION_ERROR', 'invalid scheduledAt');
...
export const moveNewsToDraft = async (req: Request, res: Response) => {
  const item = await NewsItemModel.findById(req.params.id).lean();
  if (!item) return sendError(res, 'NOT_FOUND', 'News item not found', 404);
...
export const duplicatePublishAnyway = async (req: Request, res: Response) => {
...
export const duplicateMerge = async (req: Request, res: Response) => {
  const sourceItem = await NewsItemModel.findById(req.params.id).lean();
  if (!sourceItem) return sendError(res, 'NOT_FOUND', 'News item not found', 404);
  ...
  if (!targetItem) return sendError(res, 'NOT_FOUND', 'Target news not found', 404);
  ...
  const updatedTarget = await NewsItemModel.findByIdAndUpdate(
    targetNewsId,
    {
...
export const listRssSources = async (_req: Request, res: Response) => {
...
export const createRssSource = async (req: Request, res: Response) => {
  const payload = {
    ...
    categoryTags: Array.isArray(req.body?.categoryTags) ? req.body.categoryTags.map((x: unknown) => String(x).trim()).filter(Boolean) : [],
    enabled: Boolean(req.body?.enabled ?? true),
    fetchIntervalMinutes: Number(req.body?.fetchIntervalMinutes || 30),
    priority: Number(req.body?.priority || 0)
  };

  if (!payload.name || !payload.rssUrl) return sendError(res, 'VALIDATION_ERROR', 'name and rssUrl are required');
...
export const updateRssSource = async (req: Request, res: Response) => {
  ...
  if (!before) return sendError(res, 'NOT_FOUND', 'Source not found', 404);
  ...
  if (req.body?.categoryTags !== undefined) {
    patch.categoryTags = Array.isArray(req.body.categoryTags)
      ? req.body.categoryTags.map((x: unknown) => String(x).trim()).filter(Boolean)
      : [];
  }
...
export const deleteRssSource = async (req: Request, res: Response) => {
  ...
  if (!deleted) return sendError(res, 'NOT_FOUND', 'Source not found', 404);
  await writeNewsAuditLog(req, 'delete', 'rss_source', req.params.id, { before: deleted });
  return sendSuccess(res, { deleted: true });
};

export const testRssSource = async (req: Request, res: Response) => {
  const source = await RssSourceModel.findById(req.params.id).lean();
  if (!source) return sendError(res, 'NOT_FOUND', 'Source not found', 404);
  try {
...
export const fetchRssNow = async (req: Request, res: Response) => {
  const sourceIds = Array.isArray(req.body?.sourceIds) ? req.body.sourceIds.map((id: unknown) => String(id)) : [];
...
export const putNewsSettings = async (req: Request, res: Response) => {
...
export const exportNews = async (req: Request, res: Response) => {
  ...
  const filter: Record<string, unknown> = {};
  if (status && status !== 'all') filter.status = ensureStatus(status, 'published');
  if (sourceId && mongoose.isValidObjectId(sourceId)) filter.sourceId = new mongoose.Types.ObjectId(sourceId);
...
export const getAdminNewsSettings = async (_req: Request, res: Response) => {
  const settings = await getNewsSettings();
  return sendSuccess(res, settings);
};
