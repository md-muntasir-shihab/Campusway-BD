...
export const getAdminSiteSettings = async (_req, res) => sendSuccess(res, await SiteSettingsModel.findOne().lean());
export const getAdminHomeSettings = async (_req, res) => sendSuccess(res, await HomeSettingsModel.findOne().lean());
export const putAdminSiteSettings = async (req, res) => {
    const parsed = siteSettingsSchema.safeParse(req.body);
    if (!parsed.success)
        return sendError(res, 'VALIDATION_ERROR', parsed.error.issues[0]?.message ?? 'Invalid payload');
    const saved = await SiteSettingsModel.findOneAndUpdate({}, parsed.data, { upsert: true, new: true });
    return sendSuccess(res, saved);
};
export const putAdminHomeSettings = async (req, res) => {
...
export const createBanner = async (req, res) => sendSuccess(res, await BannerModel.create(req.body), 201);
export const listBanners = async (_req, res) => sendSuccess(res, await BannerModel.find().lean());
export const updateBanner = async (req, res) => sendSuccess(res, await BannerModel.findByIdAndUpdate(req.params.id, req.body, { new: true }));
