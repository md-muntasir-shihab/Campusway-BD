...
const computeDuplicateKeyHash = (guid, url, title) => {
    if (guid)
        return normalizedHash(`guid:${guid.trim().toLowerCase()}`);
    const canonical = canonicalizeUrl(url);
    if (canonical)
        return normalizedHash(`url:${canonical}`);
...
export const runRssIngestionForSources = async (sourceIds, forceNow = false) => {
...
export const runScheduledPublishing = async () => {
    ...
    const result = await NewsItemModel.updateMany({ _id: { $in: ids } }, { $set: { status: 'published', publishedAt: now, scheduledAt: null } });
