    ...
    try {
        ...
        if (role === 'student') {
            const profile = await StudentProfile.create({
                user_id: user._id,
                user_unique_id: body.user_unique_id || `CW-${Date.now().toString().slice(-6)}-${crypto.randomInt(0, 10000).toString().padStart(4, "0")}`,
                full_name: fullName,
                username,
                email,
...
async function processImportRow(
    ...
    try {
        ...
        const profile = await StudentProfile.create({
            user_id: user._id,
            user_unique_id: row.user_unique_id || `CW-${Date.now().toString().slice(-6)}-${crypto.randomInt(0, 10000).toString().padStart(4, "0")}`,
            full_name: fullName,
            username,
            email,
...
export async function adminCreateStudent(req: AuthRequest, res: Response): Promise<void> {
    try {
        ...
        const profile = await StudentProfile.create({
            user_id: user._id,
            user_unique_id: userUniqueId || `CW-${Date.now().toString().slice(-6)}-${crypto.randomInt(0, 10000).toString().padStart(4, "0")}`,
            full_name: fullName,
            username,
            email,
            phone: phoneNumber,
