...
export default function OtpVerificationPage() {
  const { pending2FA, isAuthenticated } = useAuth();
  ...
  useEffect(() => {
    if (!pending2FA && !isAuthenticated) {
      navigate(loginRoute, { replace: true });
    }
  }, [pending2FA, isAuthenticated, navigate, loginRoute]);

  if (!pending2FA && !isAuthenticated) return null;
