        ...
        <AdminGuardShell title="Testimonials & Partners" description="Manage student reviews, approvals, featured content, and partner logos.">
            <div className="space-y-6">
                ...
                <div className="rounded-2xl border border-slate-700/30 bg-slate-900/50 p-4 flex flex-wrap gap-3 items-end">
                    <div className="flex-1 min-w-[200px] relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" aria-hidden="true" /><input type="search" value={filter.q} onChange={e => setFilter(p => ({ ...p, q: e.target.value }))} placeholder="Search..." aria-label="Search testimonials" className={`${inp} pl-9`} /></div>
                    <select value={filter.status} onChange={e => setFilter(p => ({ ...p, status: e.target.value }))} aria-label="Filter by status" className={inp + ' w-36'}><option value="all">All Status</option>{STATUSES.map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}</select>
                    <select value={filter.category} onChange={e => setFilter(p => ({ ...p, category: e.target.value }))} aria-label="Filter by category" className={inp + ' w-36'}><option value="all">All Categories</option>{CATEGORIES.map(c => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}</select>
                </div>
                        ...
                        : items.length === 0 ? <div className="p-12 text-center text-slate-500">No testimonials found</div>
                            : <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 p-4">{items.map(t => (
                                <div key={t._id} className="group relative rounded-2xl border border-slate-700/30 bg-slate-950/60 p-5 hover:border-indigo-500/25 transition-all duration-300">
                                    ...
                                    {/* Rating */}
                                    <div className="flex gap-0.5 mb-3" role="img" aria-label={`Rated ${t.rating} out of 5 stars`}>{Array.from({ length: 5 }).map((_, i) => <Star key={i} aria-hidden="true" className={`w-3.5 h-3.5 ${i < t.rating ? 'text-amber-400 fill-amber-400' : 'text-slate-700'}`} />)}</div>
                                    ...
                                    <div className="flex items-center gap-1.5 pt-3 border-t border-slate-700/20">
                                        <button onClick={() => openEdit(t)} className="flex-1 rounded-lg bg-indigo-500/10 hover:bg-indigo-500/20 py-1.5 text-[11px] font-semibold text-indigo-300 flex items-center justify-center gap-1 transition-colors"><Edit3 className="w-3 h-3" aria-hidden="true" />Edit</button>
                                        {t.status !== 'approved' && <button onClick={() => void handleApprove(t._id)} className="flex-1 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 py-1.5 text-[11px] font-semibold text-emerald-300 flex items-center justify-center gap-1 transition-colors"><CheckCircle className="w-3 h-3" aria-hidden="true" />Approve</button>}
                                        {t.status !== 'rejected' && <button onClick={() => void handleReject(t._id)} aria-label={`Reject testimonial from ${t.name}`} className="rounded-lg bg-rose-500/10 hover:bg-rose-500/20 px-2.5 py-1.5 text-[11px] font-semibold text-rose-300 flex items-center justify-center gap-1 transition-colors"><XCircle className="w-3 h-3" aria-hidden="true" /></button>}
                                        <button onClick={() => void handleToggleFeatured(t._id)} aria-label={t.featured ? `Unfeature testimonial from ${t.name}` : `Feature testimonial from ${t.name}`} aria-pressed={t.featured} className={`rounded-lg px-2.5 py-1.5 text-[11px] font-semibold flex items-center justify-center gap-1 transition-colors ${t.featured ? 'bg-amber-500/15 hover:bg-amber-500/25 text-amber-300' : 'bg-slate-700/30 hover:bg-slate-700/50 text-slate-400'}`}><Award className="w-3 h-3" aria-hidden="true" /></button>
                                        <button onClick={() => void handleDelete(t._id)} aria-label={`Delete testimonial from ${t.name}`} className="rounded-lg bg-rose-500/8 hover:bg-rose-500/15 px-2.5 py-1.5 text-[11px] font-semibold text-rose-400/70 hover:text-rose-400 flex items-center justify-center gap-1 transition-colors"><Trash2 className="w-3 h-3" aria-hidden="true" /></button>
                                    </div>
                                </div>
                            ))}</div>}
                </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setModal(null)}>
                        <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-700/30 bg-slate-900 p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
                            <div className="flex items-center justify-between mb-5">
                                ...
                                <button onClick={() => setModal(null)} aria-label="Close dialog" className="rounded-full p-2 text-slate-400 hover:bg-slate-800"><X className="w-4 h-4" aria-hidden="true" /></button>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Name *</label><input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} aria-label="Name" className={inp} /></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Role</label><input value={form.role} onChange={e => setForm(p => ({ ...p, role: e.target.value }))} aria-label="Role" className={inp} /></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">University</label><input value={form.university} onChange={e => setForm(p => ({ ...p, university: e.target.value }))} aria-label="University" className={inp} /></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Department</label><input value={form.department} onChange={e => setForm(p => ({ ...p, department: e.target.value }))} aria-label="Department" className={inp} /></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Batch</label><input value={form.batch} onChange={e => setForm(p => ({ ...p, batch: e.target.value }))} aria-label="Batch" className={inp} /></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Location</label><input value={form.location} onChange={e => setForm(p => ({ ...p, location: e.target.value }))} aria-label="Location" className={inp} /></div>
                                ...
                                <div className="sm:col-span-2"><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Or paste Avatar URL</label><input value={form.avatarUrl} onChange={e => setForm(p => ({ ...p, avatarUrl: e.target.value }))} aria-label="Avatar URL" className={inp} placeholder="https://..." /></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Rating</label><select value={form.rating} onChange={e => setForm(p => ({ ...p, rating: Number(e.target.value) }))} aria-label="Rating" className={inp}>{[5, 4, 3, 2, 1].map(r => <option key={r} value={r}>{r} Star{r > 1 ? 's' : ''}</option>)}</select></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Category</label><select value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))} aria-label="Category" className={inp}>{CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Status</label><select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value }))} aria-label="Status" className={inp}>{STATUSES.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Social Proof Label</label><input value={form.socialProofLabel} onChange={e => setForm(p => ({ ...p, socialProofLabel: e.target.value }))} aria-label="Social proof label" className={inp} placeholder="e.g. Verified Student" /></div>
                                <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Order</label><input type="number" value={form.displayOrder} onChange={e => setForm(p => ({ ...p, displayOrder: Number(e.target.value) }))} aria-label="Display order" className={inp} /></div>
                                <div className="sm:col-span-2"><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Short Quote (max 200)</label><input value={form.shortQuote} onChange={e => setForm(p => ({ ...p, shortQuote: e.target.value }))} aria-label="Short quote" className={inp} maxLength={200} /></div>
                                <div className="sm:col-span-2"><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Full Quote * <span className="normal-case tracking-normal text-slate-600">({form.fullQuote.length}/500)</span></label><textarea value={form.fullQuote} onChange={e => { if (e.target.value.length <= 500) setForm(p => ({ ...p, fullQuote: e.target.value })); }} aria-label="Full quote" className={inp + ' min-h-[100px]'} maxLength={500} /></div>
                                <div className="flex items-center gap-3"><input type="checkbox" checked={form.featured} onChange={e => setForm(p => ({ ...p, featured: e.target.checked }))} aria-label="Featured" className="rounded" /><span className="text-sm text-slate-300">Featured</span></div>
                            </div>
                    ...
                    : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">{partners.map(p => (
                        <div key={p._id} className="rounded-2xl border border-slate-700/30 bg-slate-950/50 p-4 flex flex-col items-center gap-3 hover:border-cyan-500/30 transition-colors">
                            ...
                            <div className="flex gap-1 mt-1">
                                <button onClick={() => openPEdit(p)} className="rounded-lg bg-indigo-500/15 px-2 py-1 text-[10px] font-bold text-indigo-300"><Edit3 className="w-3 h-3 inline" aria-hidden="true" /> Edit</button>
                                <button onClick={() => void handlePDelete(p._id)} aria-label={`Delete partner ${p.name}`} className="rounded-lg bg-rose-500/10 px-2 py-1 text-[10px] font-bold text-rose-400"><Trash2 className="w-3 h-3 inline" aria-hidden="true" /> Del</button>
                            </div>
                        </div>
                    ))}</div>}
        </div>
        {pModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setPModal(null)}>
                <div className="w-full max-w-md rounded-2xl border border-slate-700/30 bg-slate-900 p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center justify-between mb-5"><h3 className="text-lg font-bold text-white">{pModal === 'create' ? 'Add Partner' : 'Edit Partner'}</h3><button onClick={() => setPModal(null)} aria-label="Close dialog" className="rounded-full p-2 text-slate-400 hover:bg-slate-800"><X className="w-4 h-4" aria-hidden="true" /></button></div>
                    <div className="space-y-3">
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Name *</label><input value={pForm.name} onChange={e => setPForm(p => ({ ...p, name: e.target.value }))} aria-label="Partner name" className={inp} /></div>
                        ...
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Or paste Logo URL</label><input value={pForm.logoUrl} onChange={e => setPForm(p => ({ ...p, logoUrl: e.target.value }))} aria-label="Logo URL" className={inp} placeholder="https://..." /></div>
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Website</label><input value={pForm.websiteUrl} onChange={e => setPForm(p => ({ ...p, websiteUrl: e.target.value }))} aria-label="Website URL" className={inp} placeholder="https://..." /></div>
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Tier</label><select value={pForm.tier} onChange={e => setPForm(p => ({ ...p, tier: e.target.value }))} aria-label="Tier" className={inp}>{TIERS.map(t => <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}</select></div>
                        <div><label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block">Order</label><input type="number" value={pForm.order} onChange={e => setPForm(p => ({ ...p, order: Number(e.target.value) }))} aria-label="Display order" className={inp} /></div>
                        <div className="flex items-center gap-3"><input type="checkbox" checked={pForm.isActive} onChange={e => setPForm(p => ({ ...p, isActive: e.target.checked }))} aria-label="Active" className="rounded" /><span className="text-sm text-slate-300">Active</span></div>
                    </div>
